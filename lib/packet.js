...
exports = module.exports = function(keyParam) {
  var key;
  if (Buffer.isBuffer(keyParam)) {
    key = keyParam;
  } else if (typeof keyParam === 'string' && keyParam.match(/^[A-Fa-f0-9]{64}$/)) {
    key = new Buffer(keyParam, 'hex');
  }
  if ( !(key && key.length === 32) ) {
    throw new Error('256-bit key required. Try `head /dev/urandom | shasum -a 256`');
  }
